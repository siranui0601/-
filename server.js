...
client.on("guildCreate", (guild) => {
  ...
  .addField("サーバーの人数",`${guild.memberCount}`)
    ...
    client.channels.get('501351495267254272').send(embed);return;
});
...
`+message.content);
    ...
    if (message.content.includes('検索 暗号')) {message.channel.send('検索の少し上にあるわ！');return;}
    if (message.content.includes('検索　暗号')) {message.channel.send('検索の少し上にあるわ！');return;}
    ...
    if ( command !== '') {
        try {
          ...
          if (message.content.includes('解析')) {
            ...
            var response = function(res) { message.channel.send(res);}
          ...
          if (message.content.includes('心情')) {
            ...
            if(message.content.slice(3).match(/[\w]/)) {
              var response = function(res) { message.channel.send("この文のポジティブ度は...\n__**"+Math.round(res*100)+"%**__よ！"); }
              var logError = function(err) { console.log(err); }
              var kekka = message.content.slice(3)
              indico.sentimentHQ(kekka)
                ...
                .catch(logError);
              return;
            }else{ message.channel.send('英語にしなさいよねっ！');return;}
          }
          if (message.content.includes('R18')) {
            ...
            var indico = require('indico.io');
            indico.apiKey =  process.env.indico_api_key
            var response = function(res) { message.channel.send("この画像がR18な可能性は...\n__**"+Math.round(res*100)+"%**__よ！");
              if(Math.round(res*100) >= 70&&Math.round(res*100) < 90){message.channel.send("...おまわりさーん！こいつです！")};
              if(Math.round(res*100) >= 90&&Math.round(res*100) < 100){message.channel.send("ばかばかばかぁ！```画像を削除しました```");message.delete()};
              if(Math.round(res*100) >= 100){message.channel.send("………。```画像を削除しました```");message.delete()}}
...
どのヘルプを見たいですか？`)
          ...
          if (message.content.substring(1).startsWith('help 会話')) {//embed.setDescription('');
            var now = new Date();
            var now = new Date();
            var year = now.getFullYear();
            var month = now.getMonth();
...
約`+Math.round(cosB)+`度　　　`+sank[1]+`　　　約`+Math.round(cosC)+`度。\n面積は`+menS+`よ。これくらい暗算で頑張りなさいよね`);
 ...
 /*           message.channel.fetchMessages()
            ...
            var rireki = message.channel.fetchMessages({ limit: 10 })
...
`……ひっぐ。。。うぐっ...こ、これはっ！ただ目にゴミが……んっ`,
          ...
          if (message.content.includes('生物')) {
            message.channel.send("いくぞ、問題！w")
                var responce = [
                  'DNAの情報をもとに、タンパク質が合成されることをなんという？',
                  "ショウジョウバエ等の幼虫の~~　　~~細胞に見られる塗られる巨大な染色体をなんという？",
                  "だ腺染色体は普通の細胞の約何倍？",
                  "だ腺染色体はなんの幼虫に見られる？一つ言って",
                  "DNAの二重螺旋が解けて、転写が盛んに行われている部分をなんという？",
                  "パフとは何か",
                  "細胞が特定の形や働きをもつようになることをなんという？",
                  "分化とは何か",
                  "気温など外部の環境のことをなんという？",
                  "血液などの体内の液体の総称をなんという？",
                  "体液は細胞にとって一種の環境と考えることができる。これをなんという？",
                  "体内環境を一定に保つ性質をなんという？",
                  "脊椎動物の体液は3つに分けつことが出来る。3つ答えよ",
                  "恒常性は３つの働きで成り立っている。3つ答えよ",
                  "血液は、2つに分けることができる。何と何？",
                  "血液の無形成分の代表を答えて",
                  "血液の有形成分答えて",
                  "血球、３つ答えて",
                  "血球はどっから作られる？",
                  ...
                  "血小板たんは核を持っている？",
                  "血管に傷ができた時に、集まるのは何か",
                  "血管に傷ができた際、血小板たんによって何が生成される？",
                  "血ぺいとは何か",
                  "尿を作って排出する場所はどこ？",
                  "腎臓の働きを述べよ",
                  "肝臓の働きを述べよ",
                  "老廃物の無毒化をするのはどこ？",
                  "腎臓へ入った血液はどこからどこへろ過される？",
                  "腎臓へ入った血液が糸球体からボーマンのうへいくことをなんという？",
                  "腎臓でろ過されたものをなんという？",
                  "原尿は何管を通る？",
                  "再吸収ってなーに",
                  "原尿で再吸収されるものって何？",
                  "糸球体とボーマンのうを合わせてなんと呼ぶ？",
                  "腎小体と細尿管を合わせてなんと呼ぶ？",
                  "再吸収された原尿をなんと呼ぶ？",
                  "原尿寮の求め方は、なーに",
                  "原尿を求める手段として、使われるものは何か",
                  "原尿を求める手段として、イヌリンが使われるのはなぜか",
                  "肝臓は何が集まってできている？",
                  "肝小葉は何細胞がどのくらい集まってできている？",
                  "肝小葉の外側にある3つの管の名前を答えよ",
                  "肝動脈はどこから来たものか",
                  "肝門脈はどこから来たものか",
                  "胆管はどこからどこに向かうものか",
                  "肝小葉の中心にある脈は何か",
                  ...
                  "神経系は何から構成されている？",
                  "自律神経系は、寝ているときには働く？",
                  "交感神経は、エネルギーをどうする方向へ促進？",
                  "副交感神経は、エネルギーをどうする方向へ促進？",
                  "自律神経系の中枢はどこ？",
                  "交感神経はどういった状態のときによく働く？",
                  "副交感神経はどういった状態のときによく働く？",
                  "交感神経からは何が分泌される？",
                  "副交感神経からは何が分泌される？",
                  "　内分泌腺や神経分泌細胞でつくられるものは何？",
                  "ホルモンは何をとうして全身へ運ばれる？",
                  "どのくらいの量でホルモンは働く？",
                  "ホルモンの生成や放出を行う特定の組織や器官を何という？",
                  "放出はどこに行われる？",
                  "ホルモンは特定の器官にしか働かない。これを何という？",
                  "標的器官にはホルモンを受け取る~~　　~~を持つ標的細胞が存在する。~~　　~~は何？",
                  "ホルモンを受け取る受容体を持つ細胞を何という？",
                  "視床下部には何細胞がある？",
                  "脳下垂体は、何からなる？３つ答えて。",
                  "視床下部の神経分泌細胞が合成するホルモンによって調節されるものを？",
                  "脳下垂体後葉から放出されるものは？",
                  "バソプレシンはどこから放出される？",
                  "甲状腺から分泌されるホルモンは？",
                  "結果が原因に対して調節を行うことを？",
                  "現状の逆になるように働く調節を？",
                  "血統の調節するのどーこ",
                  "血しょう中のタンパク質の合成/分解するのどーこ",
                  "高血糖の時、視床下部は何神経を使ってどこに指示をだす？",
                  "高血糖の時、副交感神経はどこに指示を出す？",
                  "ランゲルハンス島B細胞から分泌されるホルモンは？",
                  "インスリンはどこに働く？",
                  "高血糖の時、幹細胞などの細胞は何の取り込みを促進する？",
                  "高血糖の時、幹細胞などの細胞は何の分解を促進する？",
                  "高血糖の時、幹細胞などの細胞は肝臓での何の合成を促進する？",
                  "低血糖の時、視床下部は何神経を使ってどこに指示をだす？",
                  "低血糖の時、交感神経はどこに指示を出す？",
                  "低血糖の時、視床下部は何ホルモンを放出する？",
                  "低血糖の時、視床下部が放出したホルモンを受け取るのはどこ？",
                  "低血糖の時、脳下垂体前葉が放出するホルモンは？",
                  "低血糖の時、脳下垂体前葉が放出するホルモンを受け取るのは？",
                  "低血糖の時、副腎皮質が放出するホルモンは？",
                  "低血糖の時、副腎皮質が放出するホルモンによって作用するのは？",
                  "低血糖の時、糖質コルチコイドを受け取った肝臓やその他組織は何を促進させる？",
                  "副腎髄質が放出するホルモンは？",
                  "ランゲルハンス島A細胞から分泌されるホルモンは？",
                  "グルカゴンはどこに働く？",
                  "アドレナリンはどこに働く？",
                  "糖質コルチコイドはどこに働く？",
                  "副腎皮質刺激ホルモンはどこに働く？",
                  "副腎皮質刺激ホルモン放出ホルモンはどこに働く？",
                  "低血糖の時、肝臓は何の分解を促進させる？",
                ]
                message.channel.send(responce[global.randomInt(responce.length)]);
                return;
          ...
          if (message.content.includes('日本史')) {
            message.channel.send('いくぞ、問題！w')
                var responce = [
                  '帝国議会に創設された2院を答えよ',
                  "第一回衆議院選挙は、投票率なん％？",
                  "第一回衆議院選挙は制限選挙だったどんな制限？。",
                  "1890~1894年までの議会をなんという？",
                  "初期議会とは、何年から何年？",
                  "第一回議会は何年に行われた？",
                  "第一回議会では過半数をとったのは何党？",
                  "民党には何党と何党がある？",
                  ...
                  "第一回議会での民党の方針は？",
                  "民力休養ってなーに？",
                  "第一回議会でも民党の方針の一つである経費削減ってなーに",
                  "政府はどのようにして、第一回議会を乗り切ったか",
                  "第二回議会は何年に起きたか",
                  "第二回議会の首相は誰",
                  "第二回議会の結果について樺山資紀海軍大臣が言ったものは？",
                  "蛮勇演説って？",
                  "蛮勇演説を唱えたのは？",
                  "軍の存在があったからこそ、何度かの戦争に勝てて今日国の平和があるのであって....korenalani",
                  "第二回総選挙で起きた事件は？",
                  "第四回から第六回議会を、なに内閣という？",
                  "初期議会にて民党と政府が対立した。この時に解決策を出したのは？",
                  "和協の勅令って？",
                  "和協の勅令の具体的な内容は？",
                  "公爵と侯爵はなに議員？",
                  "衆議院の牽制とは",
                  ...
                  "欧化政策の具体的内容一つ",
                  "不平等条約改正のため井上薫がした提案は？",
                  "井上薫が提案した内地解放権に＋して欧米はなにを要求した？",
                  "裁判官の半数に外国人を採用して欲しいという欧米の要求はなぜおkできない？",
                  "内地解放権とは？",
                  "「裁判官の半数に外国人を採用して欲しい」これをなに制度という？",
                  "ノルマントン号事件で辞任したのは誰？",
                  "外務大臣二人目は誰？",
                  "外国人裁判事案を推進した大隈重信は、なにがあって辞任した？",
                  "三代目外務大臣だーれ",
                  "青木周蔵が、領事裁判権を撤廃する直前に起きた事件は？",
                  "青木周蔵が辞任することになった事件は？" ,
                  "大津事件ってどんな事件？",
                  "四代目外務大臣だーれ",
                  "ニコライ二世を襲撃した犯人、津田三蔵の処罰で司法の独立を護ったのは？",
                  "小島これかたの考え方を述べて",
                  "領事裁判権を撤廃したの誰？",
                  "領事裁判権を解消した新たな条約の名は？",
                  "五代目外務大臣だーれ",
                  "関税自主権を完全に回復させたの誰？",
                  "米と交渉して関税自主権を完全に回復させ、なに条約を結んだ？",
                  "第三次伊藤内閣では、地租を何％から何％にしようとした？",
                  "立憲改進党はなんと名前を変えた？",
                  "自由党党首誰？",
                  "進歩党党首誰",
                  "増税案を否決された伊藤博文は衆議院をどうしちゃ？",
                  "自由党＋進歩党＝？",
                  "自由党と進歩党が合わさった憲政党は、何ヶ月で崩壊した？",
                  "自由党と進歩党が合わさった憲政党の首相は誰",
                  "自由党と進歩党が合わさった憲政党の内相(内務大臣)は誰",
                  "自由党と進歩党が合わさった憲政党はなに内閣と呼ばれた？",
                  "自由党と進歩党が合わさった憲政党分裂の原因は、誰のなに演説？",
                  "自由党と進歩党が合わさった憲政党の首相",
                  "共和演説について問題の部分を述べて",
                  "旧進歩党グループは憲政党解散後何党と名乗った？",
                  "旧進歩党グループは憲政党解散後何党と名乗った？",
                  "隈板内閣のあと政権は誰になった？",
                  "第二次山県有朋内閣では、集会、でも、ストライキを禁止するなにを成立させた？",
                  "治安警察法ってなに",
                  "陸・海大臣は国会議員ではなく現役対象から選ぶ。これをなんという？",
                  "第二次山県有朋内閣では、選挙資格を国税15円から何円に下げた？",
                  "1900年に伊東博文が作った新たな政党の名は？",
                  "立憲政友会は、伊東博文グループと何党が合わさった？",
                  "立憲政友会に対して反発、非難した新聞の記者は？",
                  "立憲政友会に対して反発、非難した新聞の題名は？",
                  "日清戦争の後、清の評価は何から何になったか",
                  "ロシアは租借地として清のどこをとった？",
                  "ロシアは租借地として清のなに鉄道を取った？",
                  "イギリスは租借地として清のどこをとった？",
                  "ドイツは租借地として清の何湾をとった？",
                  "フランスは租借地として清の何湾をとった？",
                  "ヨーロッパからの侵略行為に対して起こった事件を？",
                  "義和団事件の勢いを利用して清朝政府は何をした？",
                  "朝鮮王朝は清にかわってどこを頼っていた？",
                  "日露協商派の人物二人言って",
                  "ヨーロッパからの侵略行為に対して起こった事件を？",
                  "対ロシアで一致した日本と〇〇が結んだ軍事同盟を？",
                  "日英同盟を結んだのいつ？",
                  "日露戦争を、キリスト教の立場で反対した人を？",
                  "日露戦争の反対を訴えた新聞会社を？",
                  "万朝報が主戦論に転換した後退職した記者の名は？",
                  "幸徳秋水らは万朝報を止めて、新たになんという新聞を作った？",
                  "与謝野晶子が日露戦争に対して書いた詩の名前を？",
                  "「君死にたまふこと勿れ」誰が書いた？",
                  "日露戦争で陸軍はどこをとった？",
                  "日露戦争での陸軍の死者は何人以上？",
                  "日露戦争での海軍の司令官誰？",
                  ...
                  "日露戦争で条約を結んだのどこ？",
                  "日露戦争での講和条約の名前は？",
                  "ポーツマス条約で、日本全権として行ったのは誰？",
                  "ポーツマス条約で、ロシア全権として行ったのは誰？",
                  "ポーツマス条約で、仲介者として行ったのは誰？",
                  "ポーツマス条約で、ロシアは日本になんの識見があることを認めた？" ,
                  "ポーツマス条約で、ロシアはどこを？日本に譲った？",
                  "ポーツマス条約で、どっからどこまでの鉄道をもらった？",
                  "ポーツマス条約で、北緯50°以南をもらったのはどこ？",
                  "ポーツマス条約で、賠償金はいくらもらった？",
                  "ポーツマス条約で、賠償金をもらえなかったのはなぜ？",
                  "ポーツマス条約で賠償金がもらえなかったがために起きた暴動をなんという？",
                  "諸外国への韓国植民地化の承諾方法を述べよ",
                  "桂・タフト協定って何？",
                  "第二次日英同盟の内容を述べよ",
                  "第一次日韓協約の内容を述べよ",
                  "第一次日韓協約としておかれた顧問はどことどこ",
                  "第二次日韓協約の内容を述べよ",
                  "外交権を他の国に一任する国をなんという？",
                  "外交保護権を行使する役所として何をソウルに置いた？",
                  "総督府初代統監は誰",
                  "ハーブ密使事件を述べよ",
                  "第三次日韓協約の内容を述べよ",
                  "第三次日韓協定のあと国民による猛反発が起きた。これをなんという？",
                  "伊東博文はどこで誰によって殺された？",
                  "韓国合併条約とは？",
                  "統監に変わって置かれたものをなんという？",
                  "日本は韓国を植民地にした後、韓国の名前を何にした？",
                  "武断制作をしたのは誰",
                  "土地調査事業で申告漏れの土地はどうした？",
                  "土地調査事業を行った会社の名前は？",
                  "土地調査事業で個人の土地でないものはどうした？",
                  "朝鮮民衆が朝鮮総督府に対して反乱を起こしたのを何という？",
                  "三一独立運動を受け、総督府は武断ん制作をなに制作に変更したか",
                  "武断制作を文化政策に変えたのは誰",
                  "文化政策の目的を答えよ",
                  "20世紀冒頭の内閣を、なに時代という？",
                  "大逆事件で何人が逮捕された？",
                  ...
                  "軍備収縮を反発したのはどこ",
                  "陸軍のストライキとは？",
                  "陸軍のストライキができたのは、なんという法律があるから？",
                  "第一次護憲運動とは？",
                  "第一次護憲運動での反政府運動のスローガンはなに？",
                  "第一次護憲運動の後首相になったのは？",
                  "桂園時代のあと入ったのが海軍の理由は？",
                  "海軍大臣の山本権兵衛が首相になったが、これは藩閥政府でない。なぜか。",
                  "山本内閣はなにを改正させたか",
                  "山本内閣が辞任したのはなぜか",
                  "富岡製糸場はどこに払い下げられた？",
                  "長崎造船所はどこに払い下げられた？",
                  ...
                  "民営紡績会社。名前は？",
                  "大阪紡績会社の創設者は？",
                  "当時日本の輸出の約半武運はどのようなものだった？",
                  "1909年日本はなんの輸出量が世界一位になった？",
                  "生糸製造は座操製糸からどのように作るようになった？",
                  "鉄道は最初どこからの資金でできた？",
                  "主要な民営鉄道を一括運営するために出来た法律を？",
                  "日本国有鉄道、略して？",
                  "日本郵船を作った人は？",
                  "多方面にわたる企業集団の事を？",
                  "製糸工業では平均して何時間働いた？"+`幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女
幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女`,
                  "製糸工場では過酷だったが、人気があった。なぜか？"+`幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女
幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女幼女`,
                  "初めてのストライキが起きたのはどこ？",
                  "雨宮製糸工業では労働時間がどのくらい短くなった?",
                  "高野房太郎が作った労働者の全国組織をなんという？",
                  "労働者のことを重視した無産政党は何？",
                  "社会民主党の中心人物は？",
                  "社会民主党は何を目指した？",
                  "社会民主党は何法により解散させられた？",
                  "工場法では、最低労働年齢を何歳とした？",
                  "工場法は、労働時間を12時間とした？",
                  "山県有朋が作った日本初の労働者保護法をなんという？",
                  "日本最初の公害事件をなんという？",
                  "渡良瀬川流域では1880年代からたびたび凶作だった。なぜか。",
                  "足尾銅山操業停止要求演説をしたのは誰？",
                  "政府が足尾銅山を停止しなかったので、田中正造はどうしたか",
                  "政府は、公害によって使用不能となった農村をどうしたか。",
                  "山本権兵衛のあとに首相になったのは？",
                  "第一次世界大戦は何年から何年？",
                  "第一次世界大戦で出てきた新兵器は？",
                  "日本は第一次世界大戦で、何を根拠に、どっち陣営に加担した？",
                  "日本はどこを攻撃した？",
                  "日本がヨーロッパに派遣した船の役割は？",
                  "第一次世界大戦は日本に莫大な経済的利益をもたらした。これをなんという？",
                  "日本の工業製品が第一次世界大戦中たくさん売れたのはなぜ？",
                  "第一次世界大戦中、短期間で大金持ちになる人が現れた。この人々をなんという？",
                  "第一次世界大戦の最中日本が中国に行ったことは？",
                  "21ヶ条の要求の内容として、租借権や満鉄の営業期限を何年から何年間に延長させた？",
                  "21カ条の要求の内容として、どこの租借地を日本に壌土した？",
                  "日貨排斥運動ってどんなの？",
                  "山本を継いだ大隈は、なんで辞職したの?",
                  "日貨排斥運動の責任を取って辞職した大隈の次に首相になったのは？",
                  "初めての社会主義革命はどこで起きた？",
                  "ロシア革命は誰によって起きた？",
                  "シベリア出兵で、どんな病気にかかった？",
                  "シベリア出兵でいくら使った?",
                  "シベリア出兵によって起きた騒動をなんという？",
                  "寺内内閣総辞職に至ったのは、なんの責任を取ったから？",
                  "石井・ランシング協定とは？",
                  "門戸開放って？"
                ]
                message.channel.send(responce[global.randomInt(responce.length)]);
...
if (message.content.includes('Test') && !maze_0_0_front) {
  ...
  maze_0_0_front = true;
  if(maze_0_0_front){console.log("クララが立った！")}
}
if (message.content.includes('TEST')) {
  const filter3D = m => m.content.startsWith('TEST')
  console.log("TESTを受け取った！")
  message.channel.awaitMessages(filter3D, { max: 1,flag: maze_0_0_front,errors:['flag']})
    .then(collected =>{
    console.log("内容はこれだっ！")
    ...
    return;
          ...
          if ( tosi == 1) {
            message.channel.send("何年かぐらい教えなさいよ。");return;//AIzaSyCevqQInS2glVCsLlPb8_apyIIk7EJ8X_Y
          }
...
client.on('message', message => {
  ...
  if(message.channel.name === 'スピカinfo') {
    ...
    client.channels.get('501351495267254272').send(embed2);return;
  }
  if(message.channel.id === '501351495267254272') {
    ...
    .setColor('#00fff2')
    .setFooter((year)+ "年" + (month + 1) + '月' + (date) +'日、' + (hour+9) + '時' + min +`分-${message.guild}`)
    client.channels.get(あいでぃー[0]).send(embed4);return;
  }
});
client.on('message', async message => {
